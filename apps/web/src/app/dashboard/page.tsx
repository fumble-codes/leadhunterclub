...
import { BoltIcon, ViewfinderCircleIcon, ChatBubbleLeftRightIcon, BanknotesIcon, ArrowTopRightOnSquareIcon } from '@heroicons/react/24/solid'
...
export default function DashboardPage() {
  ...
  const [distribution, setDistribution] = useState<{ label: string; count: number; color: string }[]>([])
    ...
    <main className="flex-1 overflow-y-auto px-10 py-12 relative">
      ...
      <div className="max-w-[1400px] mx-auto relative z-10">
        <div className="mb-12">
          ...
          <p className="text-text-secondary mt-2">Welcome back. Here is your pipeline at a glance.</p>
        </div>
            ...
            return (
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  ...
                  {stat.trend && (
                    <span className={`text-11 font-bold ${stat.trendUp ? 'text-accent-mint' : 'text-text-secondary'} flex items-center gap-1 bg-white/5 px-2 py-1 rounded-md`}>
                      {stat.trend}
                    </span>
                  )}
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 metallic-card p-8">
            ...
            <div className="h-[200px] flex items-end justify-between gap-4">
              {activity.length > 0 ? activity.map((data, i) => {
                ...
                return (
                  <div key={data.day} className="flex-1 flex flex-col items-center gap-4 group">
                    ...
                    <span className="text-xxs font-bold text-text-secondary uppercase tracking-widest">{data.day}</span>
                  </div>
                )
              }) : (
